    ...
    <?php
        if(isset($_POST['submit_vote']))
        {
            ...
            if($insert_vote_result == '1')
            {
                ...
                $update_voter = "UPDATE voters SET has_voted = 1 WHERE voters_id = '$voter_id'";
                $update_voter_result = mysqli_query($con, $update_voter);
                if($update_voter_result) {
                    echo '<script>alert("Voted Successfully")</script>';
                    echo '<script>';
                    echo 'window.location="'.$base_url.'vlogout.php"';
                    echo '</script>';
                } else {
                    echo '<script>alert("Failed to Update Voting Status")</script>';
                }
            }
            else
            {
